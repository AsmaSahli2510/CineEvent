...
const loadStripePromise = () => {
  ...
  return stripePromise || Promise.reject(new Error("Stripe not loaded"));
};
...
function getSeatTypeFromRow(row, seatNumber) {
  ...
  if (Object.prototype.hasOwnProperty.call(seatOverrides, seatKey)) {
    return seatOverrides[seatKey];
  }
...
function findRowBySeatId(rows = [], seatId = "") {
  return rows.find((row, index) => {
    const rowPrefix = `${row?.id || row?.label || String.fromCharCode(65 + index)}-`;
    return seatId.startsWith(rowPrefix);
  });
}
...
function StripePaymentForm({
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedSeats.length === 0 || !guestInfo.fullName || !guestInfo.email) {
    ...
    try {
      ...
      if (paymentIntent.status === "succeeded") {
        const confirmResponse = await confirmGuestReservation({
          eventId: event.id,
          selectedSeats,
          totalPrice: ticketSubtotal,
        ...
        navigate(
          `/reservation-confirmation/${confirmResponse.reservation.id}`,
          {
            state: {
              bookingReference: confirmResponse.reservation.bookingReference,
              email: confirmResponse.reservation.email,
              seats: confirmResponse.reservation.selectedSeats,
              totalPrice: confirmResponse.reservation.totalPrice,
              paidAmount: confirmResponse.reservation.paidAmount,
              bookingFee: confirmResponse.reservation.bookingFee,
            },
          },
        );
      } else {
          ...
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-12">
            <div className="space-y-10 lg:col-span-7">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur-md">
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <input
                      className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none focus:border-accent focus:ring-accent disabled:opacity-50"
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur-md">
                ...
                <div className="mb-8">
                  <div className="relative flex items-center rounded-xl border-2 border-accent bg-accent/5 p-4">
                ...
                <div className="overflow-hidden rounded-2xl border border-primary/40 bg-primary/20">
                  ...
                  <div className="space-y-6 p-6">
                    ...
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-xl bg-accent py-4 font-black text-charcoal shadow-lg shadow-accent/20 transition-all hover:bg-accent/90 disabled:cursor-not-allowed disabled:opacity-50"
                    >
...
export default function GuestReservationPage() {
  ...
  const selectedSeatDetails = useMemo(() => {
    if (!venueTemplate?.rows?.length || selectedSeats.length === 0) return [];

    return selectedSeats
      .map((seatId) => {
        const row = findRowBySeatId(venueTemplate.rows, seatId);
        if (!row) return null;

        const seatNumber = Number(seatId.split("-")[1]);
        const seatType = getSeatTypeFromRow(row, seatNumber);
        const price = getSeatPrice(row, seatNumber);

        return {
          seatId,
          seatNumber,
          rowLabel: row.label || String(seatId).split("-")[0],
          seatType,
          price,
        };
      })
        ...
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12">
          <div className="space-y-10 lg:col-span-7">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur-md">
              ...
              <div className="mb-6 grid grid-cols-2 gap-3 text-[11px] md:grid-cols-6">
                ...
                <div className="flex items-center gap-2">
                  <div className="h-4 w-4 rounded-t border border-gray-600 bg-gray-600 opacity-50" />
              ...
              {venueTemplate?.rows && venueTemplate.rows.length > 0 ? (
                <div className="space-y-0">
                  {venueTemplate.rows.map((row, rowIndex) => {
                    ...
                    const rowPrefix =
                      row.id || row.label || String.fromCharCode(65 + rowIndex);

                    return (
                      <div
                        key={`row-${rowPrefix}`}
                        className={`rounded-lg border p-4 transition-all ${
                          !selectedSeats.some((s) => s.startsWith(`${rowPrefix}-`))
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <span
                            className={`w-8 flex-shrink-0 text-center font-mono text-sm font-black leading-none ${
                              ROW_TEXT_CLASS[zone] || "text-white"
                            }`}
                          >
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            {Array.from({ length: Number(row.seats) || 0 }).map(
                              (_, seatIndex) => {
                                const seatNumber = seatIndex + 1;
                                const seatType = getSeatTypeFromRow(row, seatNumber);
                                const seatId = buildSeatId(row, seatNumber, rowIndex);
                                const seatPrice = getSeatPrice(row, seatNumber);
                                ...
                                return (
                                  <button
                                    ...
                                    className={`h-7 w-7 rounded-t-md border text-[9px] font-bold transition-all ${
                                      isReserved
                                        ? "cursor-not-allowed border-gray-600 bg-gray-600 opacity-50"
                                        : isSelected
                                    ...
                                    title={
                                      isReserved
                                        ...
                                        : `${row.label || String.fromCharCode(65 + rowIndex)}-${seatNumber} (${seatType}: ${seatPrice.toFixed(2)} TND)`
                                    }
                                  >
                                    {seatNumber}
                                  </button>
                  ...
                  <button
                    onClick={() => {
                      if (
                        ...
                        alert(
                          "Please select seats and fill in your contact information",
                        );
                        return;
                      }
                      setStep("payment");

...
const STEPS = [
  ...
  },
  {
    ...
    subtitle: "Beneficiary and donation settings",
  },
  {
    id: 5,
...
const STEPPER_ITEMS = [
  ...
  { id: 4, label: "Charity", icon: "favorite" },
  { id: 5, label: "Media", icon: "photo_library" },
];

const CREATE_CHARITY_EVENT_DRAFT_STORAGE_KEY =
  "organizer_create_charity_event_draft_v1";
...
export default function CreateCharityEventPage() {
  ...
  const donationAmount = useMemo(() => {
    const percentage = Number(form.donationPercentage) || 0;
    if (percentage <= 0) return 0;
    return Number((percentage / 100).toFixed(3));
  ...
  useEffect(() => {
    try {
      const rawDraft = localStorage.getItem(
        CREATE_CHARITY_EVENT_DRAFT_STORAGE_KEY,
      );
      if (!rawDraft) {
  ...
  useEffect(() => {
    return () => {
      ...
      if (form.festivalPosterPreviewUrl) {
        URL.revokeObjectURL(form.festivalPosterPreviewUrl);
      }
  ...
  const handleFestivalPosterUpload = (event) => {
    const file = event.target.files?.[0] || null;
    setError("");
    ...
    setForm((previous) => {
      if (previous.festivalPosterPreviewUrl) {
        URL.revokeObjectURL(previous.festivalPosterPreviewUrl);
      }
  ...
  const validateStep4 = () => {
    ...
    if (!form.charityDescription.trim()) {
      return "Charity description is required.";
    }
  ...
  const clearDraft = () => {
    localStorage.removeItem(CREATE_CHARITY_EVENT_DRAFT_STORAGE_KEY);
  ...
  const handleSubmitForValidation = async () => {
    ...
    const payload = {
      ...
      charity: {
        isCharityEvent: true,
        beneficiaryAssociation: form.charityAssociation,
        minimumDonationSuggestion: Number(form.minimumDonationSuggestion),
        charityDescription: form.charityDescription.trim(),
        donationPercentage: Number(form.donationPercentage) || 0,
      },
      media: {
    ...
    try {
      ...
      await createOrganizerEvent(payload, posterFile);
    } catch (submitError) {
    ...
    setError("");
    setSuccess(
      "Charity event submitted for admin validation. It is now pending review.",
    );
  };
      ...
      return (
        <div className="space-y-6">
          <p className="text-sm text-white/70">
            Choose the type of charity event you want to create. All proceeds
            will go to the selected charity.
          </p>
            ...
            <div className="md:col-span-2 rounded-2xl border border-white/10 bg-background-dark/70 p-4">
              <div className="grid gap-5 md:grid-cols-[minmax(0,1fr)_220px]">
                <label className="space-y-2">
                  ...
                  <input
                    accept="image/jpeg,image/png"
                    onChange={handleFestivalPosterUpload}
                    className="block w-full rounded-xl border border-dashed border-white/20 bg-background-dark p-3 text-sm text-white/80 file:mr-4 file:rounded-lg file:border-0 file:bg-accent file:px-3 file:py-2 file:text-xs file:font-black file:text-charcoal"
                    type="file"
                  />
                  <p className="text-xs text-white/40">
                        ...
                        <button
                          ...
                          onClick={() => {
                            const params = new URLSearchParams({
                              templateId: venue.id,
                              preview: "1",
                              returnTo: "/organizer/events/create-charity",
                            });
                            navigate(
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-accent/30 bg-accent/10 p-4">
            ...
            <p className="mt-2 text-sm text-white/80">
              All ticket proceeds will go directly to the selected beneficiary
              association.
            </p>
          </div>
          ...
          <label className="space-y-2 md:col-span-2">
            ...
            <p className="text-xs text-white/50">
              Explain the cause and impact of this charity event.
            </p>
            <textarea
              name="charityDescription"
              value={form.charityDescription}
              onChange={handleTextInput}
              className="min-h-32 w-full rounded-xl border border-white/10 bg-background-dark px-4 py-3 text-sm text-white outline-none transition focus:border-accent"
              placeholder="Describe the charity cause and how the funds will be used..."
            />
          </label>

          <label className="space-y-2">
            <span className="text-sm font-semibold text-white/80">
              Donation percentage of revenue (%)
            </span>
            <p className="text-xs text-white/50">
              Optional. Set a specific percentage of ticket sales for charity.
              Leave empty for all proceeds.
            </p>
            <input
  ...
  return (
    <OrganizerPageFrame
      ...
      subtitle="Multi-step workflow for charity event creation">
          ...
          <aside className="space-y-4">
            ...
            <article className="rounded-2xl border border-accent/30 bg-accent/10 p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm text-white/85">
                <li>All proceeds support your chosen association.</li>
                <li>Transparent donation tracking.</li>
                <li>Impact-driven community events.</li>
              </ul>
            </article>
            ...
            <article className="rounded-2xl border border-white/10 bg-background-dark/70 p-5">
              ...
              <div className="mt-3 space-y-3 text-sm text-white/80">
                ...
                <p>
                  <span className="text-white/50">Min. donation:</span>{" "}

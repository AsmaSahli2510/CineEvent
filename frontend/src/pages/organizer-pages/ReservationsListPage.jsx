...
export default function ReservationsListPage() {
  ...
  const groupedReservations = useMemo(() => {
    ...
    reservations.forEach((reservation) => {
      if (reservation.event) {
        ...
        const eventTitle =
          reservation.event.movieDetails?.title ||
  ...
  const totalSeatsReserved = reservations.reduce(
    (sum, res) => sum + (res.selectedSeats?.length || 0),
    0,
  );
      ...
      <div className="space-y-8">
        ...
        {!loading && filteredGroups.length === 0 && (
          <div className="text-center py-20 bg-white/5 border border-white/10 rounded-lg">
            <div className="text-white/40 font-medium mb-2">
          ...
          filteredGroups.map((group) => (
            <div
              ...
              <div
                onClick={() =>
                  setExpandedEventId(
                    expandedEventId === group.eventId ? null : group.eventId,
                  )
                }
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div>
                        ...
                        <p className="text-white font-medium text-sm">
                          {formatDateTime(group.eventDate, group.eventTime)}
                        </p>
                      </div>
                      <div>
                        <p className="text-white/40 text-xs font-semibold uppercase tracking-wider mb-1">
                      ...
                      <div>
                        ...
                        <p className="text-white font-medium text-sm">
                          {group.reservations.reduce(
                            (sum, r) => sum + (r.selectedSeats?.length || 0),
                            0,
                          )}
                        </p>
                      </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-black/60 border-b border-white/10">
                          <th className="px-6 py-4 text-left text-xs font-bold text-white/50 uppercase tracking-wider">
                            Guest Name
                          </th>
                          <th className="px-6 py-4 text-left text-xs font-bold text-white/50 uppercase tracking-wider">
                            Email
                          </th>
                          <th className="px-6 py-4 text-center text-xs font-bold text-white/50 uppercase tracking-wider">
                            Seats
                          </th>
                          <th className="px-6 py-4 text-right text-xs font-bold text-white/50 uppercase tracking-wider">
                            Total Price
                          </th>
                          <th className="px-6 py-4 text-center text-xs font-bold text-white/50 uppercase tracking-wider">
                            Status
                      ...
                      <tbody className="divide-y divide-white/5">
                        {group.reservations.map((reservation) => (
                          <tr
                            ...
                            <td className="px-6 py-4 text-center">
                              <span
                                className={`inline-block px-3 py-1.5 rounded text-xs font-semibold ${statusBadgeClass(reservation.status)}`}>
                                {reservation.status?.charAt(0).toUpperCase() +
                                  reservation.status?.slice(1) || "Active"}
                              </span>
                            </td>
                            <td className="px-6 py-4">
